...
def create_or_update_table(df, table_name):
    ...
    if existing_table:
        ...
        if existing_cols == new_cols:
            ...
            counter = 1
            while DynamicTable.query.filter_by(table_name=f"{base_name}_{counter}").first():
                counter += 1
        ...
        try:
            if filename.endswith('.csv'):
                ...
                for sheet_name in excel_file.sheet_names:
                    ...
                    if not df.empty:
                        table_name = f"{filename.rsplit('.', 1)[0]}_{sheet_name}".replace(' ', '_')
...
def search():
    ...
    limit = request.args.get('limit', 1000)  # Default 1000, frontend'den kontrol edilebilir
    try:
        limit = int(limit)
    except:
        limit = 1000
