...
class DatabaseCLI:
        ...
        try:
            ...
            for table in tables:
                table_data.append([
                    table['name'],
                    table['record_count'],
                    ', '.join(table['columns'][:3]) + ('...' if len(table['columns']) > 3 else '')
                ])
    ...
    def upload_file(self, filepath):
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.print_success(f"File uploaded! Created tables: {', '.join(data['tables'])}")
            else:
    ...
    def delete_table(self, table_name):
        try:
            confirm = click.confirm(f"Are you sure you want to delete the table '{table_name}'?")
            if not confirm:
                return
...
Commands:
  ...
  exit, quit, q       - Exit
                ...
                cli.print_error(f"Unknown command: {command}. Type 'help' to see available commands.")
